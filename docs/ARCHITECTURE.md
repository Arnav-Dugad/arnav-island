...
**Phase 5G modules.**
- **`Productivity/ShareService`, protocol 2.** A transfer is an offer (file count, total, title), then per file a header (size, relative path), data and an end frame with its SHA-256, then a batch end, all sealed. Music handoff is its own mode (`H`) with an optional file in the same framing. Every socket wait goes through `select` in 200 ms slices with the transfer's stop flag, since a blocking call doesn't wake on `shutdown`. `safeSharePath` cleans received paths.
- **`Media/IslandPlayer`** wraps `IMFMediaEngine` (audio only) and `ISystemMediaTransportControls` for the island window, with the toolchain's `windows.media.h` (the duplicate `IReference<boolean>` is skipped by its guard). Engine and button events come back as `PlayerMessage`. `IslandWindow::updateSessions` puts the island's session first and drops its own copy that Windows reports (source `ArnavIsland.exe`). `mediaCommand` and `mediaSeek` route controls to it or to Windows' sessions.
- **`Media/MusicLibrary`** scans on a worker with the shell property store and thumbnails; `Media/Library.h` is the pure part (sorting, search, matching a handed-off song, shuffles).
- **`Island/IslandMusic.cpp`** holds the library view, the player and handoff (four routes on the receiving PC, then an app-session wait with `HandoffWaitTimer`).
- **Held alerts.** `holdCard` intercepts the five places cards appear; `promoteCard` brings the next forward (from the activity timer, any transition to compact, or a click on the bud). The bud's shape is `budShape` in `MotionEngine.h`, used by `Renderer::animate`, `GlassBackdrop::layout` (part 4, expressions over `b`, `bw` and `bh`), the shadow window and `updateRegion`.
- **`Renderer::swipeFollow` / `swipeEnd`** (the skip chip), `placeNav` (the liquid pill's caps and middle), `Halo` (Island DJ) and `updateBud` (the bud's label).
- **Clipboard persistence.** `saveHistory` / `loadHistory` in `ClipboardModel.h` (pure), `dibToPng` / `pngToDib` in `Clipboard.cpp`, and `IslandWindow::clipsChanged`, `saveClipHistory` and `loadClipHistory` (IslandCapture.cpp).
- **`Audio/Sounds.h`** synthesises both sounds as 16-bit WAVs in memory for `PlaySound`.


**Phase 5H modules.**
- **`Productivity/ShareService`, revision 1.** Discovery announces `port;2.1` (0.16 reads the protocol and ignores the rest; peers carry a `revision`). A music offer may carry the cover after a zero byte (never to revision 0). Two new modes: `L` lists this PC's Shelf (`frameShelf`: whether it's open, then per item its size, whether it's a folder, its name and a preview) and `T` takes one item by its place and name. Both are allowed only from paired PCs with their paired key. The taker answers the offer like a person would (1, or 2 when Downloads has no room); `offerBatch` and `takeBatch` are shared with ordinary sends.
- **`Media/CoverCodec.h`** encodes a picture as a small JPEG (PNG when it has transparent parts) and decodes one from another PC, with its palette.
- **`Media/IslandPlayer`** has two decks (`IMFMediaEngine` each). A deck fading out only hears its end. `tick` runs the equal-power ramps and starts a crossfade near a song's end, and tells the island how soon to call again (30 ms while ramping, 60–500 ms while waiting). `upNext`, `move` and `jump` edit the queue after the song playing.
- **`Island/IslandMusic.cpp`** drives `playerTick` on a timer only while needed. `fadeOutForHandoff` fades the island's song, or another app's volume in the `SessionMixer`, then pauses it. `upNextRows`, `upNextAction` and `dropQueueRow` run Up next; the renderer glides rows toward their slots per song while one is dragged.
- **Side by side.** A `spread` spring in `MotionEngine` and `budSpreadShape` (the bud, mixed from below the pill to beside it). They're applied in `Renderer::animate` (the body's offset, the bud's clip and label), `GlassBackdrop::layout` (properties `sp` and `ss` in the pill's edges and the bud's geometry), and `IslandWindow::bodyAt` / `budNow` (hit tests and the window region).
- **`Island/IslandAccess.cpp`** is the UI Automation provider: `access::Root` (fragment root, ExpandCollapse) and `access::Item` (Invoke, Toggle or RangeValue by action), built from `Renderer::accessibleTargets`. Providers run on the island's thread; anything that changes the island is posted back (`AccessMessage`). `accessChanged`, called after each redraw, raises notification events for alerts, pages, songs and levels, and marks the tree stale when it changes.
