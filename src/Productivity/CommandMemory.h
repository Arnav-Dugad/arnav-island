...
public:
    ...
    static bool memorable(CommandKind k){switch(k){case CommandKind::None:case CommandKind::ClipPaste:case CommandKind::Currency:case CommandKind::SaveWorkspace:case CommandKind::DeleteWorkspace:case CommandKind::ClearClipboard:return false;default:return true;}}
    ...
    static CommandMemory read(std::istream& in){
        ...
        while(std::getline(in,line)){if(!line.empty()&&line.back()=='\r')line.pop_back();std::vector<std::string> f;size_t a=0;for(;;){size_t b=line.find('\t',a);f.push_back(line.substr(a,b==std::string::npos?std::string::npos:b-a));if(b==std::string::npos)break;a=b+1;}
            ...
            try{RememberedCommand r;const int kind=std::stoi(f[0]);if(kind<=0||kind>int(CommandKind::Currency))continue;r.kind=CommandKind(kind);r.value=std::stoi(f[1]);r.last=std::stoll(f[2]);r.count=std::max(0,std::stoi(f[3]));r.pinned=f[4]=="1";
