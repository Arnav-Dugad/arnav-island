...
// One HTTPS GET, at most 1 MB, five-second timeouts.
std::optional<std::string> httpGet(const wchar_t* host,const wchar_t* path){
    const wchar_t* agent=L"ArnavIsland (https://github.com/Arnav-Dugad/arnav-island)";
    Internet session{WinHttpOpen(agent,WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,WINHTTP_NO_PROXY_NAME,WINHTTP_NO_PROXY_BYPASS,0)};
    ...
    Internet connection{WinHttpConnect(session.h,host,INTERNET_DEFAULT_HTTPS_PORT,0)};if(!connection.h)return std::nullopt;
    Internet request{WinHttpOpenRequest(connection.h,L"GET",path,nullptr,WINHTTP_NO_REFERER,WINHTTP_DEFAULT_ACCEPT_TYPES,WINHTTP_FLAG_SECURE)};if(!request.h)return std::nullopt;
