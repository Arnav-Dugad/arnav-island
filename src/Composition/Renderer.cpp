...
void Renderer::redraw(const ContentSnapshot& s,bool debug,bool headerOnly) {
    ...
    updateArtwork(s,raised);updateRings(s,accent,muted,line);
    ...
    surface(contentSurface_,380,284,[&](auto* rt){
        ...
        if(s.page==Page::Overview){
            ...
            double position=std::clamp(s.playback.position+(s.playback.playing?std::max(0.,seconds()-s.playback.sampledAt):0.),0.,s.playback.duration);box(0,191,380,2,line,1);if(s.playback.duration>0)box(0,191,float(380*position/s.playback.duration),2,accent,1);text(rt,s.playback.duration>0?clockText(position):L"No timeline provided",0,200,160,10,muted);button(Action::MediaMode,s.settings.mediaLayout==0?L"Auto":s.settings.mediaLayout==1?L"Music":L"Video",167,199,54,22);text(rt,s.playback.duration>0?clockText(s.playback.duration):L"",300,200,80,10,muted,DWRITE_FONT_WEIGHT_NORMAL,DWRITE_TEXT_ALIGNMENT_TRAILING);
