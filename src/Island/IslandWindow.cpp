...
int IslandWindow::run(HINSTANCE instance,const std::wstring& cmd){
    ...
    audio_=std::make_unique<AudioProvider>(window_);if(cmd.find(L"--capture-safe")==std::wstring::npos)media_=std::make_unique<MediaProvider>(window_);system_=std::make_unique<SystemProvider>(window_);if(cmd.find(L"--capture-safe")==std::wstring::npos){analyzer_=std::make_unique<LoopbackAnalyzer>(window_);mixer_=std::make_unique<SessionMixer>(window_);brightness_=std::make_unique<BrightnessProvider>(window_);}content_.settings=settings_;
    ...
    if(testing_&&cmd.find(L"--qa-showcase")!=std::wstring::npos){
        ...
        MediaSnapshot second=first;second.title=L"Browser playback study";second.artist=L"Synthetic QA session";second.service=MediaService::YouTube;second.browser=true;second.appName=L"Microsoft Edge";second.appIcon=browser.icon;second.playing=false;second.current=false;second.source=L"qa.browser";
        content_.sessions={first,second};content_.session=0;content_.playback=first;content_.media=first.title;selectedSource_=first.source;
...
IslandWindow::~IslandWindow(){
    ...
    previews_.reset();brightness_.reset();mixer_.reset();analyzer_.reset();system_.reset();media_.reset();audio_.reset();for(auto h:powerNotifications_)UnregisterPowerSettingNotification(h);
...
void IslandWindow::animate(){
    ...
    bool artwork=content_.playback.artwork&&(expanded?(content_.live||content_.page==Page::Media||content_.page==Page::Overview):settings_.compactMedia&&!content_.hud);
...
void IslandWindow::transition(IslandState s){bool changed=state_!=s;state_=s;if(s!=IslandState::Compact&&content_.hud){content_.hud=0;motion_.compactWidth=settings_.uiMode==0?72:settings_.compactWidth;}content_.expanded=s!=IslandState::Compact;content_.live=s==IslandState::LiveActivity;if(changed){content_.settings=settings_;if(content_.expanded)refresh();else renderer_->redraw(content_,debug_,true);}clockTimer();animate();if(lab_)InvalidateRect(lab_,nullptr,FALSE);}
...
LRESULT IslandWindow::message(UINT m,WPARAM w,LPARAM l){
    ...
    case WM_TIMER: if(w==17){KillTimer(window_,17);fullscreen();return 0;}
        ...
        if(w==SettleTimer){double t=seconds();if(motion_.width.settled(t)&&motion_.height.settled(t)&&motion_.dragX.settled(t)&&motion_.dragY.settled(t)){updateRegion(false);renderer_->animate(motion_,t);KillTimer(window_,SettleTimer);}else updateRegion(true);}
...
void IslandWindow::perform(Action a){
    ...
    if(a>=Action::Overview&&a<=Action::Focus){if(content_.page!=Page(int(a)-int(Action::Overview))&&!motion_.reduced){motion_.contentShift.reset(5,now);motion_.contentShift.retarget(0,now,MotionTokens::content);}content_.page=Page(int(a)-int(Action::Overview));transition(IslandState::Expanded);}
    ...
    if(inRange(a,Action::SessionBase,Action::ActionEnd)){switchSession(int(a)-int(Action::SessionBase),true);return;}
    ...
    case Action::Play:if(media_&&content_.playback.canToggle)media_->control(1,content_.playback.source);break;
    case Action::Previous:if(media_&&content_.playback.canPrevious)media_->control(2,content_.playback.source);break;
    case Action::Next:if(media_&&content_.playback.canNext)media_->control(3,content_.playback.source);break;
