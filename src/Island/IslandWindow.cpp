...
int IslandWindow::run(HINSTANCE instance,const std::wstring& cmd){
    // What this run was started with (minus an update's own two), to start the next version the same way. A test run's
    // update flags stay behind: the version it starts is what it says it is, so it doesn't update itself again.
    {std::wistringstream words(cmd);std::wstring w;while(words>>w){if(w.rfind(L"--after=",0)==0)continue;if(w.rfind(L"--qa-version=",0)==0||w==L"--qa-update"){launchArgsQa_=true;continue;}if(w.rfind(L"--updated=",0)==0){updatedFrom_=w.substr(10);continue;}launchArgs_+=(launchArgs_.empty()?L"":L" ")+w;}}
    ...
    if(auto at=cmd.find(L"--qa-weather");testing_&&at!=std::wstring::npos){int code=3;auto eq=cmd.find(L'=',at);if(eq!=std::wstring::npos&&eq<cmd.find(L' ',at))code=_wtoi(cmd.c_str()+eq+1);settings_.weather=true;settings_.homeMetrics={0,7,8};content_.weather={true,14.4,code,cmd.find(L"--qa-night")==std::wstring::npos,L"Sample town"};
        ...
        if(auto a=cmd.find(L"--qa-aqi=");a!=std::wstring::npos)content_.weather.now.aqi=_wtof(cmd.c_str()+a+9);
    ...
    // --qa-settings-sweep (with --qa-settings=0): every Settings section saved as drawn, into the temp folder's arnav-settings-sweep.
    if(testing_&&cmd.find(L"--qa-settings-sweep")!=std::wstring::npos&&settingsWindow_){const auto dir=std::filesystem::temp_directory_path()/L"arnav-settings-sweep";std::error_code e;std::filesystem::remove_all(dir,e);std::filesystem::create_directories(dir,e);settingsWindow_->sweep(dir.wstring());}
    ...
    if(testing_&&cmd.find(L"--qa-health-chart")!=std::wstring::npos)content_.batteryHealthChart=true;
    // --qa-whats-new: the Updated card with sample notes (as a real update brings them).
    if(testing_&&cmd.find(L"--qa-whats-new")!=std::wstring::npos){content_.whatsNew=whatsNewLines("# Arnav Island\n## Updates itself\n- **Automatic updates.** New versions arrive on their own.\n  1. Downloads it\n  2. Checks it\n## Weather\n- **Weather on the glass.** Rain runs down the island, drops bead on it and merge as they meet, and fog drifts across it when your town has them.\n- **Hourly curve.** A smooth line through the next eight hours.\n## Battery\n- **Health over time.** A chart of your battery's health, marking where it fell below 90% and 80%.\n## Checks\n- Everything passed.\n");updatedFrom_=L"0.18.0-preview.2";if(cmd.find(L"--qa-whats-new-sheet")!=std::wstring::npos)perform(Action::WhatsNewOpen);else SetTimer(window_,UpdatedTimer,900,nullptr);}
    // --qa-verify=<file>: which publisher signed a program (the updater's own check), into verify-qa.txt.
    if(auto at=cmd.find(L"--qa-verify=");testing_&&at!=std::wstring::npos){std::wstring file=cmd.substr(at+12);if(auto space=file.find(L" --");space!=std::wstring::npos)file.resize(space);if(file.size()>1&&file.front()==L'"')file=file.substr(1,file.find(L'"',1)-1);
        std::ofstream(store_.directory/L"verify-qa.txt")<<"signer="<<signerSha256(file)<<"\npublisher="<<(signedByPublisher(file)?1:0)<<"\n";}
...
void IslandWindow::transition(IslandState s){
    ...
    if(changed&&s!=IslandState::Notification&&motion_.bud.target()!=0){content_.bud={};const double t=seconds();if(motion_.reduced)motion_.bud.reset(0,t);else motion_.bud.retarget(0,t,SpringSpec{1,320,36});}if(s!=IslandState::Compact&&content_.hud){content_.hud=0;motion_.compactWidth=settings_.uiMode==0?72:settings_.compactWidth;}content_.expanded=s!=IslandState::Compact;if(!content_.expanded){content_.weatherView=false;content_.whatsNewView=false;}content_.live=s==IslandState::LiveActivity||s==IslandState::Notification||s==IslandState::Command;content_.card=s==IslandState::Notification||s==IslandState::Command;
...
LRESULT IslandWindow::message(UINT m,WPARAM w,LPARAM l){
    ...
    case WM_MOUSEWHEEL:
        ...
        if(state_==IslandState::Expanded&&content_.page==Page::Overview&&content_.whatsNewView){const int step=GET_WHEEL_DELTA_WPARAM(w)>0?-2:2;content_.whatsNewOffset=std::clamp(content_.whatsNewOffset+step,0,std::max(0,int(content_.whatsNew.size())-renderer_->whatsNewShown_));refresh();return 0;}
    ...
    case WM_TIMER: if(w==17){KillTimer(window_,17);fullscreen();return 0;}if(w==SpreadTimer){KillTimer(window_,SpreadTimer);spreadAlerts(false);return 0;}if(w==69&&testing_){KillTimer(window_,69);spreadAlerts(true);return 0;}if(w==72&&testing_){KillTimer(window_,72);renderer_->pointer(24,14,true,true);return 0;}if(w==71&&testing_){KillTimer(window_,71);shareCard(qaLateCard_==17?17:15,qaLateCard_==17?L"Blue in Green":L"Studio PC",qaLateCard_==17?L"Miles Davis  ·  from Studio PC":L"Holiday photos  ·  12 files  ·  48.2 MB",{},60);return 0;}if(w==66&&testing_){KillTimer(window_,66);perform(Action::Shelf);return 0;}if(w==63){KillTimer(window_,63);syncBud();return 0;}if(w==68&&testing_){KillTimer(window_,68);const double t=seconds();renderer_->swipeFollow(-52,float(motion_.width.sample(t).position),float(motion_.height.sample(t).position),true,false);return 0;}if(w==67&&testing_){KillTimer(window_,67);shareCard(16,L"Received from Travel laptop",L"Trip notes.pdf",{},8);return 0;}
        ...
        if(w==13){
            ...
            case 20:{RECT box{};GetWindowRgnBox(window_,&box);const double r=motion_.radius.sample(seconds()).position,scale=dpi_/96;
                ...
                pass=!settings_.floating()&&(!renderer_->glassAvailable()||renderer_->glassStyle().visible)&&topRow>middleRow+int(2*r*scale)&&renderer_->glassError().empty();
            ...
            case 44:{auto has=[&](Action a){return std::any_of(renderer_->targets.begin(),renderer_->targets.end(),[&](auto& t){return t.action==a;});};
                ...
                settings_.sharing=false;content_.settings=settings_;content_.nearby.clear();content_.pinned=false;perform(Action::Close);SetTimer(window_,13,500,nullptr);break;}
            // 0.17.0-preview.2: Clear glass through every shape: open, the command bar, two alerts dropping and spreading,
            // back to compact, then Frosted the same way. The glass must follow each (no error from its expressions).
            case 45:{settings_.material=2;settings_.edge=0;applySettings(false,true);content_.page=Page::Media;content_.pinned=true;transition(IslandState::Expanded);SetTimer(window_,13,700,nullptr);break;}
            case 46:{content_.pinned=false;perform(Action::Close);openCommand();SetTimer(window_,13,700,nullptr);break;}
            case 47:{closeCommand(false);settings_.notifyStyle=1;settings_.stackAlerts=true;heldCards_.clear();shareCard(16,L"UI test first",L"One",{},30);shareCard(16,L"UI test second",L"Two",{},30);spreadAlerts(true);SetTimer(window_,13,1400,nullptr);break;}
            case 48:{spreadAlerts(false);heldCards_.clear();content_.bud={};syncBud();events_.dismiss(seconds());content_.activity.clear();transition(IslandState::Compact);settings_.material=1;applySettings(false,true);
...
void IslandWindow::perform(Action a){
    ...
    case Action::BatteryChart:content_.batteryHealthChart=!content_.batteryHealthChart;break;
    case Action::WhatsNewOpen:if(content_.whatsNew.empty())break;events_.dismiss(now);content_.activity.clear();content_.whatsNewView=true;content_.whatsNewOffset=0;content_.page=Page::Overview;content_.pinned=true;transition(IslandState::Expanded);break;
    case Action::WhatsNewBack:content_.whatsNewView=false;if(!motion_.reduced){motion_.swipe.reset(-14.,now);motion_.swipe.retarget(0,now,MotionTokens::content);}break;
