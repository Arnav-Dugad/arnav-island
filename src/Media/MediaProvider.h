...
// simultaneous players can be browsed. Commands address a session by its app ID.
class MediaProvider {
    HWND window_;HANDLE stop_;std::shared_ptr<void> changed_;std::thread worker_;
    std::mutex mutex_;std::vector<MediaSnapshot> sessions_;struct Command{int action;std::wstring source;};std::vector<Command> commands_;double requestedSeek_=-1;std::wstring seekSource_,seekTitle_;
...
public:
    ...
    void seek(double seconds,const MediaSnapshot& expected){{std::lock_guard lock(mutex_);requestedSeek_=seconds;seekSource_=expected.source;seekTitle_=expected.title;}SetEvent(changed_.get());}
    void control(int action,const std::wstring& source={}){{std::lock_guard lock(mutex_);commands_.push_back({action,source});}SetEvent(changed_.get());}
};
}
