...
std::shared_ptr<const Artwork> readArtwork(Properties* properties,HANDLE stop){
    ...
    auto art=std::make_shared<Artwork>();art->width=w;art->height=h;art->pixels.resize(size_t(w)*h*4);check(converter->CopyPixels(nullptr,w*4,UINT(art->pixels.size()),art->pixels.data()));return art;
}
}
