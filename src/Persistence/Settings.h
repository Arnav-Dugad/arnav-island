...
#define FLAG(name) else if(key==#name)s.name=value!=0;
        ...
        FLAG(waveform) FLAG(appIcons) FLAG(hud) FLAG(followSession) FLAG(autoHide) FLAG(alertsReveal) FLAG(deviceCards) FLAG(powerCards) FLAG(batteryHistory) FLAG(clipboardHistory) FLAG(clipboardConfirm) FLAG(privacyDots) FLAG(privacyCards)
#undef FLAG
...
#define WRITE(name) out<<#name<<' '<<name<<'\n';
        ...
        WRITE(waveform) WRITE(appIcons) WRITE(hud) WRITE(followSession) WRITE(autoHide) WRITE(alertsReveal) WRITE(deviceCards) WRITE(powerCards) WRITE(batteryHistory) WRITE(clipboardHistory) WRITE(clipboardConfirm) WRITE(privacyDots) WRITE(privacyCards) WRITE(commandShortcut)
#undef WRITE
    }
