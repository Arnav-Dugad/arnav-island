...
namespace nexus {
struct Settings {
    int version=2,preset=0,monitor=0,verticalOffset=0,horizontalOffset=0,hoverDelay=220,mediaLayout=0;
    bool reduceMotion=false,hideFullscreen=true,hoverOpen=true;
    static Settings parse(std::istream& in) {
        Settings s;std::string key;double value;
        while(in>>key) {
            if(!(in>>value))throw std::runtime_error("Missing setting value");
            if(!std::isfinite(value))throw std::runtime_error("Nonfinite setting");
            if(key=="version") {if(value!=1&&value!=2)throw std::runtime_error("Unsupported settings version");}
            ...
            else if(key=="hoverOpen")s.hoverOpen=value!=0;
            else if(key=="hoverDelay")s.hoverDelay=int(std::clamp(value,100.,700.));
            else if(key=="mediaLayout")s.mediaLayout=int(std::clamp(value,0.,2.));
        }
