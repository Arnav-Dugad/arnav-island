...
#define WRITE(name) out<<#name<<' '<<name<<'\n';
        ...
        WRITE(waveform) WRITE(appIcons) WRITE(hud) WRITE(followSession) WRITE(autoHide) WRITE(alertsReveal) WRITE(deviceCards) WRITE(powerCards) WRITE(batteryHistory) WRITE(clipboardHistory) WRITE(clipboardConfirm) WRITE(privacyDots) WRITE(privacyCards) WRITE(springStiffness) WRITE(springDamping) WRITE(springMass) WRITE(commandShortcut) WRITE(waveTimeline) WRITE(lyrics) WRITE(lyricsCompact) WRITE(headphoneCards) WRITE(captureShortcuts) WRITE(hideSecrets) WRITE(pinnedShelf) WRITE(currency) WRITE(commandHistory)
#undef WRITE
    }
